...
# FIX DATES , add server side? add client side? probably server side
def newOrder(customer_id):
    with engine.connect() as conn:
        newOrder= { 
            "customer_id" : customer_id,
            "date" :  "2024-01-16 18:57:21.000",
            "orderStatus":"processessing"
            }
    
    
        conn.execute(sqlalchemy.insert(Order),newOrder)
        conn.commit()

    # add a
    pass
# pass customer id here? or validate 
def updateOrder(id,book_id,quantity):
    with engine.connect() as conn:
        newOrderItem= { 
        ...
        result = conn.execute(sqlalchemy.insert(orderItem),newOrderItem)
        conn.commit()
